...
class Environment:
    ...
    def get_repo(self,start_date,end_date,codes_num,market):
        ...
        #read all data
        self.data=pd.read_csv(r'./data/'+market+'.csv', parse_dates=True,dtype=object)
        ...
        while sample_flag:
            ...
            print(len(self.data.loc[self.data["code"].isin([codes[0]])]), len(self.data.loc[self.data["code"].isin([codes[1]])]))

            # 生成有效时间
            date_set=set(data2.loc[data2['code']==codes[0]].index)
            print("length of date_set before is ", len(date_set), ". codes is ",codes)
            for code in codes:
                # 
                date_set=date_set.intersection((set(data2.loc[data2['code']==code].index)))
                print("-> length of date_set after is ", len(date_set), code)
